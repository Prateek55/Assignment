...
with open("pos_word_corpus") as pos_corpus_file:  # SenticNet positive polarity concepts
	for line in pos_corpus_file:
		word=line[:-1]
		pos_corpus.append(word)
collision_file=open("tweet_collision","w") # the File has been renamed now

with open('negative-words.txt') as tweet_neg_corpus: # New Twitter data set, came classified as positive and negative beforehand
